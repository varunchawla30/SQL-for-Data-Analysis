...
select ipl_bidding_details.bidder_id, (count(*)/no_of_bids)*100 as Win_Percentage from 
((ipl_bidder_details inner join ipl_bidding_details on ipl_bidder_details.bidder_id = ipl_bidding_details.bidder_id)
inner join ipl_bidder_points on ipl_bidder_details.bidder_id = ipl_bidder_points.bidder_id)
where ipl_bidding_details.bid_status = 'Won'
group by ipl_bidder_details.bidder_id
order by Win_Percentage desc;

# Question 2 - Display the number of matches conducted at each stadium with stadium name, city from the database.

select stadium_name, city, count(*) matches_played from 
ipl_stadium join ipl_match_schedule on ipl_stadium.stadium_id = ipl_match_schedule.stadium_id 
...
# Question 4 - Show the total bids along with bid team and team name.

select ipl_bidding_details.BID_TEAM, team_name, count(no_of_bids) as total_bids from 
(ipl_bidding_details join ipl_bidder_points on ipl_bidding_details.BIDDER_ID = ipl_bidder_points.BIDDER_ID) join 
ipl_team on ipl_bidding_details.bid_team = ipl_team.team_id
group by bid_team, team_name;


# Question 5 - Show the team id who won the match as per the win details.

update ipl_match set remarks=substr(WIN_DETAILS,6,3);
select match_id, substr(WIN_DETAILS,6,3) as team_won, ipl_team.team_id from ipl_match join ipl_team 
on ipl_match.remarks = ipl_team.remarks;

# Question 6 - Display total matches played, total matches won and total matches lost by team along with its team name.

select team_name, sum(matches_played) as matches_played, sum(matches_won) as matches_won, 
sum(matches_lost) as matches_lost from ipl_team_standings join ipl_team
on ipl_team_standings.team_id = ipl_team.team_id group by ipl_team_standings.team_id;


# Question 7 - Display the bowlers for Mumbai Indians team.

select player_name from ((IPL_Team_players join IPL_Team on IPL_Team_players.TEAM_ID = IPL_Team.TEAM_ID)
join IPL_Player on IPL_Team_players.player_id=IPL_Player.player_id)
where player_role  ='Bowler' and team_name='Mumbai Indians';

# Question 8 - How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.

select team_name,count(*) as number_of_all_rounders from (IPL_Team_players join IPL_Team on IPL_Team_players.TEAM_ID = IPL_Team.TEAM_ID)
join IPL_Player on IPL_Team_players.player_id=IPL_Player.player_id
where player_role  ='All-Rounder' group by team_name;

select team_name,count(*) as number_of_all_rounders from (IPL_Team_players join IPL_Team on IPL_Team_players.TEAM_ID = IPL_Team.TEAM_ID)
join IPL_Player on IPL_Team_players.player_id=IPL_Player.player_id
where player_role  ='All-Rounder' group by team_name having number_of_all_rounders>4 order by number_of_all_rounders desc;